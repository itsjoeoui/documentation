---
title: Modbus
description: Using NominalModbus for Modbus TCP and RTU device communication
---
...
modbus_device = NominalModbus(config="my_device.json", autostart=True)
...
The typical NominalModbus workflow follows this pattern:

1. **Define your device** — Create a JSON config file or build a `ModbusConfig` in Python
2. **`NominalModbus(config)`** — Instantiate the client with your config (and optionally a separate `connection`)
...
Modbus devices can vary in how they order bytes within multi-register values. NominalModbus provides three swap options to handle these differences:
...
NominalModbus is configured through a JSON file. Every config file must include:
...
Modbus TCP connections drop for all the usual reasons — network blips, device reboots, power cycles on a PLC, etc. NominalModbus handles this transparently: when a read or write hits a transport error, the driver closes the dead socket and pymodbus opens a fresh one on the next operation. No custom retry loop, no explicit `open()` after a disconnect.
...
device = NominalModbus(config="my_device.json")
device.open()
```
...
device = NominalModbus(config="my_device.json", connection=connection)
device.open()
```
...
device = NominalModbus(config="temperature_controller.json")
device.open()
...
<Note>
Channel names in the returned `Measurement` and `Command` objects follow the pattern `{device_name}.{register_alias}`. Write commands append `.cmd` to the channel name.
</Note>
...
device = NominalModbus(config="heat_exchanger.json", autostart=True)
...
device = NominalModbus(config="heat_exchanger.json")
device.add_publisher(NominalCorePublisher(dataset_rid="ri.datasets.main.dataset.abc123"))
...
f4t = NominalModbus("watlow_f4t.json", connection=connection, autostart=True)
...
| Method | Purpose |
|--------|---------|
| `NominalModbus(config, connection=None, name=None, publishers=None, autostart=False)` | Create a new Modbus client from a JSON config file, `dict`, or `ModbusConfig` instance. Optionally pass a separate connection. |
...
NominalModbus raises descriptive errors for common issues:
