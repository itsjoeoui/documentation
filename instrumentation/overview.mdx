...
Here's the same workflow using the Nominal Instrumentation library's `NominalDAQ`.
...
daq = NominalDAQ.create(
    name="myDAQ",
    vendor=DAQVendor.LABJACK_T_SERIES, # Swap out vendor with DAQVendor.NI for NI DAQ Devices
    resource="<your_device_id>"
    )
...
Instrument-specific pages (e.g. [NominalDAQ](/instrumentation/daq)) describe the exact methods and timing options for each type.
...
* `NominalPSU`: programmable Power Supplies
* `NominalDAQ`: data Acquisition devices
* `NominalELoad`: electronic Loads
* `NominalI2C`: I2C master for communicating to peripherals on an I2C bus
