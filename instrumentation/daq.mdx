...
description: Using NominalDAQ for vendor-independent data acquisition
---
...
NominalDAQ is a hardware abstraction layer (HAL) that provides a unified interface for data acquisition devices from multiple vendors. The key benefit is **vendor-independent code**: change the `DAQVendor` parameter and the same code works across different hardware.
...
The typical NominalDAQ workflow follow this pattern:

1. **`NominalDAQ.create()`** - Instantiate the DAQ with vendor and resource info
...
NominalDAQ supports both analog and digital I/O:
...
NominalDAQ supports two acquisition modes:
...
## Creating a NominalDAQ Instance

Use the `create()` factory method to instantiate a NominalDAQ:

```python
from nominal_instro.instruments import NominalDAQ
from nominal_instro.instruments.daq.types import DAQVendor

# Create a LabJack DAQ
daq = NominalDAQ.create(
    name="myDAQ",
    vendor=DAQVendor.LABJACK_T_SERIES,
    resource="440020473"  # Vendor specific resource identifier
)
```
...
- **`vendor`**: The DAQ vendor (NI, LABJACK_T_SERIES, or KEYSIGHT_34980)
- **`resource`**: Device identifier
  - For NI and LabJack: string (device name, serial number, or IP address. Vendor specific resource identifier.)
  - For Keysight (SCPI device): `ConnectConfig` object with VISA resource string
- **`publishers`**: List of publishers to attach
- **`dataset_rid`**: Shorthand to add a `NominalCorePublisher`

### Vendor-Specific Resource Examples
...
# Keysight (SCPI/VISA device)
from nominal_instro.lib import ConnectConfig

daq = NominalDAQ.create(
    name="keysightDAQ",
    vendor=DAQVendor.KEYSIGHT_34980,
    resource=ConnectConfig.visa("TCPIP0::<IP_ADDRESS>::INSTR")
)
```
...
`NominalDAQ` supports adding a `Scaler` object when you configure your analog channel. The `Measurement` published and returned by `read_analog` will contain these scaled values.

Example of a 0-5V pressure sensor that measures 0-3000 psia.
```python
from nominal_instro.instruments.daq.types import Direction
from nominal_instro.instruments.daq.scaling import LinearScaler
...
For example, a thermocouple that's fed into an amplifier and then the DAQ will require two stages of scaling.
 ...
 ```python
 from nominal_instro.instruments.daq.scaling import ScalerPipeline, ReverseLinearScaler
 from nominal_instro.instruments.daq.scaling.thermocouple import ThermocoupleSensor
...
scaler_pipeline = ScalerPipeline(ReverseLinearScaler(gain=<amplifier gain>, offset=<voltage offset>, units="V"), ThermocoupleSensor(type = <TC_type>, cjc_temp = <cjc temp in Celsius>))
 ...
 ```python
from nominal_instro.instruments.daq.scaling import Scaler
...
- **physical_channel**: The physical line or port on your DAQ device (e.g., "5101", "5101/2"). This name is vendor-specific—refer to your device documentation.
...
```python
from nominal_instro.instruments.daq.types import Direction, DigitalPortWidth, Logic
...
- **Multiplexed DAQs** (e.g., LabJack T4/T7, most NI DAQ devices): Maximum per-channel rate decreases with more channels
...
| Method | Purpose |
|--------|---------|
| `create(name, vendor, resource, publishers=None, **kwargs)` | Factory method to create NominalDAQ instance |
...
The power of NominalDAQ is that **the same code works across different vendors**. To switch vendors, just change the `vendor` and `resource` parameters:
...
This section is for developers implementing NominalDAQ support for DAQ devices / vendors that are not supported out of the box.

## Overview

Driver developers implement the [`DAQDriverBase` abstract interface](#daqdriverbase-interface) to add support for new DAQ vendors. The driver is responsible for **translating NominalDAQ's vendor-independent API calls into vendor-specific hardware operations**, ensuring users get consistent behavior regardless of the underlying hardware (within its capabilities).
...
3. **Timing abstraction**: Implement both software-timed reads and hardware-timed buffered acquisition. `NominalDAQ` will manage the background thread for the hardware-timed acquisition.
...
- **`_read_to_measurements(response, channel_list, daq_name, **kwargs) -> list[Measurement]`**: Convert vendor data format to NominalDAQ `Measurement` objects
...
Drivers must handle one of two types of resource identifiers:

- **`device_id` (string)**: For devices accessed via vendor-specific libraries (e.g., NI-DAQmx, LabJack LJM)
  - Examples: Device names ("Dev1"), serial numbers ("440020473"), IP addresses

- **`ConnectConfig` (SCPI/VISA)**: For devices accessed via SCPI/VISA communication
  - Must implement `DAQDriverDataHandler` interface, which is a subclass of `DAQDriverBase`
  - Use injected `DataHandler` for SCPI command/query operations

### NominalDAQ State Information

Drivers have access to the state information `NominalDAQ` via the `APINominalDAQ` interface available as an attribute, `self.hal`. `NominalDAQ` keeps track of the settings a user has configured so that the driver doesn't have to keep track of state except where necessary due to vendor implementation details.

```python
class APINominalDAQ(Protocol):
    """API for the NominalDAQ instrument type."""
    @property
...
#### Example: Vendor Difference Solved with APINominalDAQ

Different vendors configure timing differently, but NominalDAQ users must always call `configure_ai_sample_rate()` before `start()`.
...
- **LabJack LJM**: Sample rate is configured when `ljm.eStreamStart()` is called. Since `NominalDAQ` stores the configuration information when `NominalDAQ.configure_ai_sample_rate()` is called, the `start()` implementation can grab the `HWTimingConfig` from `NominalDAQ` and configure the device.

**Driver's Job**: Implement these differences so the user interface is consistent at the `NominalDAQ` level.

## Summary

Driver development requires careful abstraction of vendor-specific behaviors to provide the unified NominalDAQ interface. Focus on:
