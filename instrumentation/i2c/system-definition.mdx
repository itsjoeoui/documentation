...
description: Understanding SystemDefinition for NominalI2C configuration
---
...
When you create a `NominalI2C` instrument, you must provide a `SystemDefinition` that describes:
...
The `SystemDefinition` enables NominalI2C to:
...
Access devices by name when using NominalI2C methods:
...
NominalI2C supports defining two types of I2C devices in the `SystemDefinition`.
...
- **`name`**: Human-readable identifier used in NominalI2C method calls
...
```python
from nominal_instro.instruments.i2c.types import DataFormat
...
```python
from nominal_instro.instruments.i2c.types import DataFormat, FieldDef
...
```python
from nominal_instro.instruments.i2c.types import FieldDef
...
When reading or writing fields, NominalI2C automatically handles the masking and bit shifting:
...
```python
from nominal_instro.instruments.i2c.types import CommandDevice, CommandDef, DataFormat
...
When you call `i2c.query("adc", "read_ch0_fast")`, NominalI2C automatically ORs `ADCChannel.CH0 | FastMode.ENABLE` and sends the result.
...
```python
from nominal_instro.instruments.i2c.types import DataFormat, LinearScaling
...
```python
from nominal_instro.instruments.i2c.types import LinearScaling
...
```python
from nominal_instro.instruments.i2c.types import CustomScaling
...
You can reuse a `SystemDefinition` across multiple `NominalI2C` instances if they share the same I2C bus configuration. Create the definition once and pass it to multiple instruments.
</Tip>
