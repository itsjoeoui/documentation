...
description: Using NominalI2C for vendor-independent I2C communication
---
...
NominalI2C is a hardware abstraction layer (HAL) that provides a unified interface for I2C communication across multiple adapter vendors. The key benefit is **vendor-independent code**: create your `SystemDefinition` once, and the same code works with different I2C adapters.
...
NominalI2C's architecture utilizes a `SystemDefinition` to provide a **low-code, human-readable way** to interact with I2C devices on the bus. This design reduces the need for magic numbers, manual bitwise operations, and scattered hardware knowledge throughout your test code.
...
See the [System Definition](/instrumentation/i2c/system-definition) page for detailed information on creating and configuring your `SystemDefinition`.
...
The typical NominalI2C workflow follows this pattern:

1. **Create a `SystemDefinition`** - Define all I2C devices, registers, and commands (see [System Definition](/instrumentation/i2c/system-definition))
2. **`NominalI2C.create()`** - Instantiate the I2C instrument with vendor and resource info
...
NominalI2C's `System Definition` supports two types of I2C devices:
...
This is useful for using I2C devices that NominalI2C doesn't yet provide lower-code interactions with, allowing you to move forward regardless.
</Note>

## Creating a NominalI2C Instance

Use the `create()` factory method to instantiate a NominalI2C instrument:

```python
from nominal_instro.instruments import NominalI2C
...
i2c = NominalI2C.create(
    ...
    resource="123456",  # Serial number or device identifier
...
- **`vendor`**: The I2C adapter vendor (currently `I2CVendor.TOTAL_PHASE`)
- **`resource`**: Vendor-specific resource identifier (serial number for Total Phase Aardvark)
...
NominalI2C requires a `SystemDefinition` parameter. You cannot create a NominalI2C instance without first defining your I2C devices. See the [System Definition](/instrumentation/i2c/system-definition) page for details.
</Warning>
...
from nominal_instro.instruments.i2c.types import (
    SystemDefinition,
    RegisterDevice,
    RegisterDef,
    FieldDef,
...
```python
from nominal_instro.instruments import NominalI2C
from nominal_instro.instruments.i2c.types import I2CVendor
...
| Method | Purpose |
|--------|---------|
| `create(name, vendor, resource, system_definition, publishers=None, **kwargs)` | Factory method to create NominalI2C instance |
...
This section is for developers implementing NominalI2C support for I2C adapter vendors that are not supported out of the box.

## Overview

Driver developers implement the `I2CDriverBase` abstract interface to add support for new I2C adapter vendors. The driver is responsible for **translating NominalI2C's vendor-independent API calls into vendor-specific hardware operations**, ensuring users get consistent behavior regardless of the underlying hardware.
...
### NominalI2C State Information

Drivers have access to the state information `NominalI2C` maintains via the `APINominalI2C` interface available as an attribute, `self.hal`. Currently, this interface is empty but may be extended in the future for driver-specific needs.

## Implementation Example: Total Phase Driver

Here's a reference implementation for part of Total Phase Aardvark driver:

```python
from nominal_instro.instruments.i2c.driver import I2CDriverBase

class AardvarkI2CDriver(I2CDriverBase):
    def __init__(self, serial_number: "str"):
        import pyaardvark  # type: ignore

        self.device = pyaardvark.open(serial_number=serial_number)

    def read(self, address: int, length: int) -> bytes:
        return self.device.i2c_master_read(address, length)

    def write(self, address: int, data: bytes):
        self.device.i2c_master_write(address, data)

    def write_read(self, address: int, data: bytes, read_len: int) -> bytes:
        return self.device.i2c_master_write_read(address, data, read_len)
    ...
...
For custom drivers, you can construct `NominalI2C` directly and pass your driver instance:

```python
from nominal_instro.instruments.i2c import NominalI2C, I2CDriverBase
from nominal_instro.instruments.i2c.types import SystemDefinition
...
# Construct NominalI2C with custom driver (not using create)
system = SystemDefinition()
