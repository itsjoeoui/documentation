---
title: "NominalModbus feature showcase"
---

```python modbus_example.py
"""Example: NominalModbus feature showcase.
...
Start the sim server first:
    python -m nominal_instro.unstable.modbus.examples.sim_server
...
from nominal_instro.unstable.modbus import NominalModbus

CONNECTION = {"transport": "tcp", "host": "127.0.0.1", "port": 5020, "unit_id": 1, "timeout": 2.0}
CONFIG_PATH = resources.files("nominal_instro.unstable.modbus.examples") / "simulated_modbus_device.json"


def main():
    device = NominalModbus(str(CONFIG_PATH), connection=CONNECTION, autostart=True)
    device.add_publisher(NominalCorePublisher("ri.catalog.cerulean-staging.dataset.056356ba-5c64-479c-9fc8-da0eba27ae0b"))
